...
var backendURL =
    "https://vitawelln7439back.builtwithrocket.new/log-inspected-widget";
...
class _CustomWidgetInspectorState extends State<CustomWidgetInspector> {
  ...
  void _handleTap() {
    if (_selectedElement != null && isInspectorEnabled) {
      try {
        ...
        debugPrint('Error sending widget info: $err || $st');
      }
    }
  }
  ...
  _listenEvent() {
    // Eliminado: lógica web para escuchar eventos. No es compatible con móvil.
  }
  ...
  void check(Element e) {
    ...
    if (json.containsKey('creationLocation')) {
      ...
      if (filePath.isNotEmpty &&
        ...
        // fileName eliminado porque no se usa
...
void _sendWidgetInformation(Map<String, dynamic> widgetInfo) {
  // En móvil, solo mostramos la información en consola.
  debugPrint('Widget info: ${jsonEncode(widgetInfo)}');
}
